...
grid.innerHTML = properties.map(p =>   <a href="./pages/property-details.html?id=${p.id}" class="property-card">   <img src="${p.image}" alt="${p.title}">   <div class="property-card-body">   <div class="property-card-price">${formatPrice(p.price)}</div>   <h3 class="property-card-title">${truncateText(p.title, 40)}</h3>   <div class="property-card-meta">${ICONS.pin}<span>${p.city}, ${p.area}</span></div>   <div class="property-card-meta">   <span>${ICONS.bed} ${p.bedrooms} Beds</span>   <span>${ICONS.bath} ${p.bathrooms} Baths</span>   </div>   </div>   </a>  ).join('');
}
...
return   <a href="./pages/property.html?city=${encodeURIComponent(city)}" class="city-card" style="--city-bg:url('${bg}')">   <div>   <h3>${city}</h3>   <p>${count} Properties</p>   </div>   </a>  ;
}).join('');
}
...
grid.innerHTML = PROPERTY_TYPES.map(type =>   <a href="./pages/property.html?type=${encodeURIComponent(type)}" class="category-card">   <div class="category-icon">${ICONS[CATEGORY_ICON_MAP[type]] || ICONS.flat}</div>   <h3>${type}</h3>   </a>  ).join('');
}
...
grid.innerHTML = points.map(p =>   <div class="why-card">   <div class="why-icon">${p.icon}</div>   <h3>${p.title}</h3>   <p>${p.desc}</p>   </div>  ).join('');
}
